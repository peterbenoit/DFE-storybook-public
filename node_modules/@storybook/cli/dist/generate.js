...
`).replace("<<type>>",isTypescript?": StorybookConfig":"").replace("<<mainContents>>",JSON.stringify(config,null,2));await import_fs_extra2.default.writeFile(`./${storybookConfigFolder}/main.${isTypescript?"ts":"js"}`,(0,import_ts_dedent.dedent)(mainJsContents),{encoding:"utf8"})}async function configurePreview(options){let{prefix=""}=options.frameworkPreviewParts||{},isTypescript=options.language==="typescript"||options.language==="typescript-legacy",previewPath=`./${options.storybookConfigFolder}/preview.${isTypescript?"ts":"js"}`;if(await import_fs_extra2.default.pathExists(previewPath))return;let preview=import_ts_dedent.dedent`
    ${prefix}
...
`.trimStart(),promptForCompoDocs=async()=>{let{useCompoDoc}=await(0,import_prompts2.default)({type:"confirm",name:"useCompoDoc",message:"Do you want to use Compodoc for documentation?"});return useCompoDoc},AngularJSON=class{constructor(){if(!import_fs4.default.existsSync(ANGULAR_JSON_PATH))throw commandLog("An angular.json file was not found in the current directory. Storybook needs it to work properly."),new Error("No angular.json file found");let jsonContent=import_fs4.default.readFileSync(ANGULAR_JSON_PATH,"utf8");this.json=JSON.parse(jsonContent)}get projects(){return this.json.projects}get projectsWithoutStorybook(){return Object.keys(this.projects).filter(projectName=>{let{architect}=this.projects[projectName];return!architect.storybook})}getProjectSettingsByName(projectName){return this.projects[projectName]}async getProjectName(){if(this.projectsWithoutStorybook.length>1){let{projectName}=await(0,import_prompts2.default)({type:"select",name:"projectName",message:"For which project do you want to generate Storybook configuration?",choices:this.projectsWithoutStorybook.map(name=>({title:name,value:name}))});return projectName}return this.projectsWithoutStorybook[0]}addStorybookEntries({angularProjectName,storybookFolder,useCompodoc,root}){let{architect}=this.projects[angularProjectName],baseOptions={configDir:storybookFolder,browserTarget:`${angularProjectName}:build`,compodoc:useCompodoc,...useCompodoc&&{compodocArgs:["-e","json","-d",root||"."]}};architect.storybook||(architect.storybook={builder:"@storybook/angular:start-storybook",options:{...baseOptions,port:6006}}),architect["build-storybook"]||(architect["build-storybook"]={builder:"@storybook/angular:build-storybook",options:{...baseOptions,outputDir:Object.keys(this.projects).length===1?"storybook-static":`dist/storybook/${angularProjectName}`}})}write(){import_fs4.default.writeFileSync(ANGULAR_JSON_PATH,JSON.stringify(this.json,null,2))}};var generator=async(packageManager,npmOptions,options,commandOptions)=>{var _a,_b;let packageJson=packageManager.retrievePackageJson(),angularVersionFromDependencies=(_a=import_semver5.default.coerce(packageManager.retrievePackageJson().dependencies["@angular/core"]))==null?void 0:_a.version,angularVersionFromDevDependencies=(_b=import_semver5.default.coerce(packageManager.retrievePackageJson().devDependencies["@angular/core"]))==null?void 0:_b.version,angularVersion=angularVersionFromDependencies||angularVersionFromDevDependencies,updatedOptions=import_semver5.default.gte(angularVersion,"12.0.0")?{...options,builder:"webpack5"}:options,angularJSON=new AngularJSON;if(angularJSON.projectsWithoutStorybook.length===0)return paddedLog("Every project in your workspace is already set up with Storybook. There is nothing to do!"),Promise.reject();let angularProjectName=await angularJSON.getProjectName();paddedLog(`Adding Storybook support to your "${angularProjectName}" project`);let{root}=angularJSON.getProjectSettingsByName(angularProjectName),{projects}=angularJSON,useCompodoc=commandOptions.yes?!0:await promptForCompoDocs(),storybookFolder=root?`${root}/.storybook`:".storybook";if(root!==""){import_fs5.default.mkdirSync(storybookFolder,{recursive:!0});let rootReferencePathFromStorybookFolder=root.split("/").map(()=>"../").join("");import_fs5.default.writeFileSync(`${storybookFolder}/main.ts`,(0,import_ts_dedent4.default)(`
    ...
    `},async run({result:{eslintFile,unsupportedExtension},packageManager,dryRun}){let deps=["eslint-plugin-storybook"];if(logger6.info(`\u2705 Adding dependencies: ${deps}`),dryRun||packageManager.addDependencies({installAsDevDependencies:!0},deps),!dryRun&&unsupportedExtension){logger6.info(import_ts_dedent10.dedent`
...
`:""}
      ...
      We've detected you are using Storybook in a ${import_chalk14.default.bold("Next.js")} project.

      In Storybook 7, we introduced a new framework package for Next.js projects: ${import_chalk14.default.magenta("@storybook/nextjs")}.

      This package is a replacement for ${import_chalk14.default.magenta("@storybook/react-webpack5")} and provides a better experience for Next.js users.
      ${addonsMessage}
      To learn more about it, see: ${import_chalk14.default.yellow("https://github.com/storybookjs/storybook/blob/next/code/frameworks/nextjs/README.md")}
    `},async run({result:{addonsToRemove,main,frameworkOptions,packageJson},packageManager,dryRun}){let dependenciesToRemove=[...addonsToRemove,"@storybook/react-webpack5"];if(dependenciesToRemove.length>0&&(logger10.info(`\u2705 Removing redundant packages: ${dependenciesToRemove.join(", ")}`),!dryRun)){packageManager.removeDependencies({skipInstall:!0,packageJson},dependenciesToRemove);let updatedAddons=main.getFieldValue(["addons"]).filter(addon=>typeof addon=="string"?!addonsToRemove.includes(addon):addon.name?!addonsToRemove.includes(addon.name):!1);main.setFieldValue(["addons"],updatedAddons)}if(logger10.info("\u2705 Installing new dependencies: @storybook/nextjs"),!dryRun){let versionToInstall=getStorybookVersionSpecifier(packageJson);packageManager.addDependencies({installAsDevDependencies:!0,packageJson},[`@storybook/nextjs@${versionToInstall}`])}logger10.info("\u2705 Updating framework field in main.js"),dryRun||(main.setFieldValue(["framework","options"],frameworkOptions),main.setFieldValue(["framework","name"],"@storybook/nextjs"),await(0,import_csf_tools5.writeConfig)(main))}};var import_chalk15=__toESM(require("chalk")),import_ts_dedent15=__toESM(require("ts-dedent")),import_semver15=__toESM(require("semver")),import_csf_tools6=require("@storybook/csf-tools"),import_core_common8=require("@storybook/core-common");var logger11=console,packagesMap={"@storybook/react":{webpack5:"@storybook/react-webpack5",vite:"@storybook/react-vite"},"@storybook/preact":{webpack5:"@storybook/preact-webpack5",vite:"@storybook/preact-vite"},"@storybook/server":{webpack5:"@storybook/server-webpack5"},"@storybook/angular":{webpack5:"@storybook/angular"},"@storybook/vue":{webpack5:"@storybook/vue-webpack5"},"@storybook/vue3":{webpack5:"@storybook/vue3-webpack5",vite:"@storybook/vue3-vite"},"@storybook/svelte":{webpack5:"@storybook/svelte-webpack5",vite:"@storybook/svelte-vite"},"@storybook/web-components":{webpack5:"@storybook/web-components-webpack5",vite:"@storybook/web-components-vite"},"@storybook/html":{webpack5:"@storybook/html-webpack5",vite:"@storybook/html-vite"}},getBuilder=builder=>typeof builder=="string"?builder.includes("vite")?"vite":"webpack5":builder!=null&&builder.name.includes("vite")?"vite":"webpack5",getFrameworkOptions=(framework,main)=>{let frameworkOptions={};try{frameworkOptions=main.getFieldValue([`${framework}Options`])}catch(e){logger11.warn(import_ts_dedent15.default`
    ...
    `),console.log(e)}return frameworkOptions||{}},newFrameworks={id:"newFrameworks",async check({packageManager}){var _a;let packageJson=packageManager.retrievePackageJson(),allDeps={...packageJson.dependencies,...packageJson.devDependencies},{mainConfig,version:storybookVersion,framework}=(0,import_core_common8.getStorybookInfo)(packageJson);if(!mainConfig)return logger11.warn("Unable to find storybook main.js config, skipping"),null;let storybookCoerced=storybookVersion&&((_a=import_semver15.default.coerce(storybookVersion))==null?void 0:_a.version);if(!storybookCoerced)throw new Error(import_ts_dedent15.default`
      ...
      `);if(!import_semver15.default.gte(storybookCoerced,"7.0.0"))return null;let main=await(0,import_csf_tools6.readConfig)(mainConfig),frameworkPackage=main.getNameFromPath(["framework"]),builder=main.getFieldValue(["core","builder"]);if(!frameworkPackage||!Object.keys(packagesMap).includes(frameworkPackage))return null;let builderInfo={name:getBuilder(builder),options:main.getFieldValue(["core","builder","options"])||{}},newFrameworkPackage=packagesMap[frameworkPackage][builderInfo.name];if(!newFrameworkPackage)return null;let frameworkOptions=newFrameworkPackage==="@storybook/svelte-vite"?{}:getFrameworkOptions(framework,main),dependenciesToRemove=["@storybook/builder-webpack5","@storybook/manager-webpack5","@storybook/builder-webpack4","@storybook/manager-webpack4","@storybook/builder-vite","storybook-builder-vite"].filter(dep=>allDeps[dep]),dependenciesToAdd=[];if(newFrameworkPackage!==frameworkPackage&&dependenciesToAdd.push(newFrameworkPackage),allDeps.vite&&import_semver15.default.lt(import_semver15.default.coerce(allDeps.vite).version,"3.0.0"))throw new Error(import_ts_dedent15.default`
        ❌ Your project should be upgraded to use the framework package ${import_chalk15.default.bold(newFrameworkPackage)}, but we detected that you are using Vite ${import_chalk15.default.bold(allDeps.vite)}, which is unsupported in ${import_chalk15.default.bold("Storybook 7.0")}. Please upgrade Vite to ${import_chalk15.default.bold("3.0.0 or higher")} and rerun this migration.
      ...
      We can remove the dependencies that are no longer needed: ${import_chalk15.default.yellow(dependenciesToRemove.join(", "))}
      
      And set up the ${import_chalk15.default.magenta(frameworkPackage)} framework that already includes the builder.

      To learn more about the framework field, see: ${import_chalk15.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#framework-field-mandatory")}

      ${import_chalk15.default.underline(import_chalk15.default.bold(import_chalk15.default.cyan("Webpack4 users")))}
...
`)}
    ...
    `}};var import_chalk17=__toESM(require("chalk")),import_ts_dedent17=require("ts-dedent"),import_path12=require("path"),import_fs_extra8=__toESM(require("fs-extra")),import_globby=__toESM(require("globby")),MDX1_STYLE_START=/<style>{`/g,MDX1_STYLE_END=/`}<\/style>/g,MDX1_COMMENT=/<!--(.+)-->/g,MDX1_CODEBLOCK=/(?:\n~~~(?:\n|.)*?\n~~~)|(?:\n```(?:\n|.)*?\n```)/g,fixMdxStyleTags=mdx=>mdx.replace(MDX1_STYLE_START,"<style>\n  {`").replace(MDX1_STYLE_END,"  `}\n</style>"),fixMdxComments=mdx=>{let codeblocks=mdx.matchAll(MDX1_CODEBLOCK);return mdx.split(MDX1_CODEBLOCK).map(v=>v.replace(MDX1_COMMENT,(original,group)=>`{/*${group}*/}`)).reduce((acc,item,i)=>{let next=codeblocks.next();return next.done?acc+item:acc+item+next.value[0]},"")},logger12=console,mdx1to2={id:"mdx1to2",async check(){let storiesMdxFiles=await(0,import_globby.default)("./!(node_modules)**/*.(story|stories).mdx");return storiesMdxFiles.length?{storiesMdxFiles}:void 0},prompt({storiesMdxFiles}){return import_ts_dedent17.dedent`
    ...
    `},async run({result:{storiesMdxFiles},dryRun}){await Promise.all([...storiesMdxFiles.map(async fname=>{let contents=await import_fs_extra8.default.readFile(fname,"utf-8"),updated=fixMdxComments(fixMdxStyleTags(contents));updated===contents?logger12.info(`\u{1F197} Unmodified ${(0,import_path12.basename)(fname)}`):(logger12.info(`\u2705 Modified ${(0,import_path12.basename)(fname)}`),dryRun||await import_fs_extra8.default.writeFile(fname,updated))})])}};var import_chalk18=__toESM(require("chalk")),import_ts_dedent18=require("ts-dedent"),import_csf_tools7=require("@storybook/csf-tools"),import_core_common10=require("@storybook/core-common"),logger13=console,autodocsTrue={id:"autodocsTrue",async check({packageManager}){let packageJson=packageManager.retrievePackageJson(),{mainConfig}=(0,import_core_common10.getStorybookInfo)(packageJson);if(!mainConfig)return logger13.warn("Unable to find storybook main.js config, skipping"),null;let main=await(0,import_csf_tools7.readConfig)(mainConfig),docs=main.getFieldValue(["docs"]),docsPageToAutodocsMapping={true:"tag",automatic:!0,false:!1};if(docs!=null&&docs.docsPage){let oldValue=docs==null?void 0:docs.docsPage.toString();if(!(oldValue in docsPageToAutodocsMapping))throw new Error(`Unexpected value for docs.docsPage: ${oldValue}`);return{main,value:docsPageToAutodocsMapping[oldValue]}}return(docs==null?void 0:docs.autodocs)===void 0?{main}:null},prompt({value}){let autodocsFormatted=import_chalk18.default.cyan(`docs: { autodocs: ${JSON.stringify(value??!0)} }`);return value?import_ts_dedent18.dedent`
      ...
      More info: ${import_chalk18.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#autodocs")}
    `:import_ts_dedent18.dedent`
      ...
      More info: ${import_chalk18.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#autodocs")}
    `},async run({result:{main,value},dryRun}){logger13.info("\u2705 Setting 'docs.autodocs' to true in main.js"),dryRun||(main.removeField(["docs","docsPage"]),main.setFieldValue(["docs","autodocs"],value??!0),await(0,import_csf_tools7.writeConfig)(main))}};var import_chalk19=__toESM(require("chalk")),import_ts_dedent19=__toESM(require("ts-dedent")),import_semver16=__toESM(require("semver")),import_csf_tools8=require("@storybook/csf-tools"),import_core_common11=require("@storybook/core-common");var logger14=console,fixId="sveltekitFramework",sveltekitFramework={id:fixId,async check({packageManager}){var _a;let packageJson=packageManager.retrievePackageJson();if(!{...packageJson.dependencies,...packageJson.devDependencies}["@sveltejs/kit"])return null;let{mainConfig,version:storybookVersion}=(0,import_core_common11.getStorybookInfo)(packageJson);if(!mainConfig)return logger14.warn("Unable to find storybook main.js config, skipping"),null;let sbVersionCoerced=storybookVersion&&((_a=import_semver16.default.coerce(storybookVersion))==null?void 0:_a.version);if(!sbVersionCoerced)throw new Error(import_ts_dedent19.default`
        ❌ Unable to determine storybook version.
        🤔 Are you running automigrate from your project directory?
      `);if(!import_semver16.default.gte(sbVersionCoerced,"7.0.0"))return null;let main=await(0,import_csf_tools8.readConfig)(mainConfig),framework=main.getNameFromPath(["framework"]);if(!framework)return logger14.warn(import_ts_dedent19.default`
      ❌ Unable to determine Storybook framework, skipping ${import_chalk19.default.cyan(fixId)} fix.
      🤔 Are you running automigrate from your project directory?
    `),null;if(framework==="@storybook/sveltekit")return null;if(framework==="@storybook/svelte-vite")return{main,packageJson,dependenciesToRemove:["@storybook/svelte-vite"]};if(framework!=="@storybook/svelte")return logger14.warn(import_ts_dedent19.default`
            ...
            If you are interested in using this package, see: ${import_chalk19.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#sveltekit-needs-the-storybooksveltekit-framework")}
          `),null;let builder=main.getFieldValue(["core","builder"]);return["@storybook/builder-vite","storybook-builder-vite"].includes(builder)?{main,packageJson,dependenciesToRemove:[builder]}:(logger14.warn(import_ts_dedent19.default`
          We've detected you are using Storybook in a SvelteKit project.
...
`);return import_ts_dedent20.dedent`
      ...
      More info: ${import_chalk20.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#react-peer-dependencies-required")}
...
`:"",manualFixesMessage=fixSummary.manual.length>0?`
 ...
 ${fixSummary.skipped.map(m=>import_chalk23.default.cyan(m)).join(", ")}
    `:"",summaryMessage=import_ts_dedent23.default`
...
`}If you'd like to run the migrations again, you can do so by running '${import_chalk23.default.cyan("npx storybook@next automigrate")}'
  ...
  `;return(0,import_boxen.default)(summaryMessage,{borderStyle:"round",padding:1,title,borderColor:hasFailures?"red":"green"})}var logger17=console,installStorybook=(projectType,packageManager,options)=>{let npmOptions={installAsDevDependencies:!0,skipInstall:options.skipInstall},packageJson;try{packageJson=packageManager.readPackageJson()}catch{}let generatorOptions={language:detectLanguage(packageJson),builder:options.builder||detectBuilder(packageManager),linkable:!!options.linkable,pnp:options.usePnp};return(async()=>{switch(projectType){case"REACT_SCRIPTS":return REACT_SCRIPTS_default(packageManager,npmOptions,generatorOptions).then(commandLog('Adding Storybook support to your "Create React App" based project'));case"REACT":return REACT_default(packageManager,npmOptions,generatorOptions).then(commandLog(`Adding Storybook support to your "React" app
`));case"REACT_NATIVE":return(options.yes?Promise.resolve({server:!0}):(0,import_prompts4.default)([{type:"confirm",name:"server",message:"Do you want to install dependencies necessary to run Storybook server? You can manually do it later by install @storybook/react-native-server",initial:!1}])).then(({server})=>REACT_NATIVE_default(packageManager,npmOptions,server)).then(commandLog(`Adding Storybook support to your "React Native" app
...
`));case"NX":return paddedLog("We have detected Nx in your project. Please use `nx g @nrwl/storybook:configuration` to add Storybook to your project."),paddedLog("For more information, please see https://nx.dev/packages/storybook"),Promise.reject();case"UNSUPPORTED":return paddedLog("We detected a project type that we don't support yet."),paddedLog("If you'd like your framework to be supported, please let use know about it at https://github.com/storybookjs/storybook/issues"),logger17.log(),Promise.resolve();default:return paddedLog(`We couldn't detect your project type. (code: ${projectType})`),paddedLog("You can specify a project type explicitly via `sb init --type <type>`, see our docs on how to configure Storybook for your framework: https://storybook.js.org/docs/react/get-started/install"),logger17.log(),projectTypeInquirer(options,packageManager)}})().catch(ex=>{logger17.error(`
     ${import_chalk24.default.red(ex.stack)}`),process.exit(1)})},projectTypeInquirer=async(options,packageManager)=>{let manualAnswer=options.yes?!0:await(0,import_prompts4.default)([{type:"confirm",name:"manual",message:"Do you want to manually choose a Storybook project type to install?"}]);if(manualAnswer!==!0&&manualAnswer.manual){let frameworkAnswer=await(0,import_prompts4.default)([{type:"select",name:"manualFramework",message:"Please choose a project type from the following list:",choices:installableProjectTypes.map(type=>({title:type,value:type.toUpperCase()}))}]);return installStorybook(frameworkAnswer.manualFramework,packageManager,options)}return Promise.resolve()};async function doInitiate(options,pkg2){let{packageManager:pkgMgr}=options;options.useNpm&&(useNpmWarning(),pkgMgr="npm");let packageManager=JsPackageManagerFactory.getPackageManager({force:pkgMgr}),welcomeMessage="storybook init - the simplest way to add a Storybook to your project.";logger17.log(import_chalk24.default.inverse(`
 ${welcomeMessage} 
`));let{default:updateNotifier}=await import("simple-update-notifier");await updateNotifier({pkg:pkg2,updateCheckInterval:1e3*60*60});let projectType,projectTypeProvided=options.type,infoText=projectTypeProvided?`Installing Storybook for user specified project type: ${projectTypeProvided}`:"Detecting project type",done=commandLog(infoText),packageJson=packageManager.retrievePackageJson();try{projectTypeProvided?installableProjectTypes.includes(projectTypeProvided)?projectType=projectTypeProvided.toUpperCase():(done(`The provided project type was not recognized by Storybook: ${projectTypeProvided}`),logger17.log(`
The project types currently supported by Storybook are:
`),installableProjectTypes.sort().forEach(framework=>paddedLog(`- ${framework}`)),logger17.log(),process.exit(1)):projectType=detect(packageJson,options)}catch(ex){done(ex.message),process.exit(1)}done();let storybookInstalled=isStorybookInstalled(packageJson,options.force);if(storybookInstalled&&projectType!=="ANGULAR"){logger17.log(),paddedLog("There seems to be a Storybook already available in this project."),paddedLog(`Apply following command to force:
`),codeLog(["sb init [options] -f"]),logger17.log();return}let installResult=await installStorybook(projectType,packageManager,options).catch(e=>{process.exit()});if(!options.skipInstall&&!storybookInstalled&&packageManager.installDependencies(),options.disableTelemetry||(0,import_telemetry.telemetry)("init",{projectType}),await automigrate({yes:options.yes||process.env.CI==="true",packageManager:pkgMgr}),logger17.log(`
To run your Storybook, type:
`),projectType==="ANGULAR"?codeLog([`ng run ${installResult.projectName}:storybook`]):codeLog([packageManager.getRunStorybookCommand()]),logger17.log(`
For more information visit:`,import_chalk24.default.cyan("https://storybook.js.org")),projectType==="REACT_NATIVE"){let REACT_NATIVE_REPO="https://github.com/storybookjs/react-native";logger17.log(),logger17.log(import_chalk24.default.red("NOTE: installation is not 100% automated.")),logger17.log(`To quickly run Storybook, replace contents of your app entry with:
`),codeLog(["export {default} from './storybook';"]),logger17.log(`
 For more in information, see the github readme:
`),logger17.log(import_chalk24.default.cyan(REACT_NATIVE_REPO)),logger17.log()}logger17.log()}async function initiate(options,pkg2){await(0,import_core_server.withTelemetry)("init",{cliOptions:options},()=>doInitiate(options,pkg2))}var import_path14=__toESM(require("path")),import_fs7=__toESM(require("fs")),import_cross_spawn3=require("cross-spawn"),import_core_common14=require("@storybook/core-common"),import_csf_tools10=require("@storybook/csf-tools");var logger18=console,LEGACY_CONFIGS=["addons","config","presets"],postinstallAddon=async(addonName,isOfficialAddon)=>{let skipMsg=null;isOfficialAddon?import_fs7.default.existsSync(".storybook")?(skipMsg="no codmods found",LEGACY_CONFIGS.forEach(config=>{try{let codemod=require.resolve(`${getPackageName(addonName,isOfficialAddon)}/postinstall/${config}.js`);commandLog(`Running postinstall script for ${addonName}`)();let configFile=import_path14.default.join(".storybook",`${config}.ts`);import_fs7.default.existsSync(configFile)||(configFile=import_path14.default.join(".storybook",`${config}.js`),import_fs7.default.existsSync(configFile)||import_fs7.default.writeFileSync(configFile,"","utf8")),(0,import_cross_spawn3.sync)("npx",["jscodeshift","-t",codemod,configFile],{stdio:"inherit",shell:!0}),skipMsg=null}catch{}})):skipMsg="no .storybook config":skipMsg="unofficial addon",skipMsg&&commandLog(`Skipping postinstall for ${addonName}, ${skipMsg}`)()},getVersionSpecifier=addon=>{let groups=/^(...*)@(.*)$/.exec(addon);return groups?[groups[1],groups[2]]:[addon,void 0]};async function add(addon,options){let{packageManager:pkgMgr}=options;options.useNpm&&(useNpmWarning(),pkgMgr="npm");let packageManager=JsPackageManagerFactory.getPackageManager({force:pkgMgr}),packageJson=packageManager.retrievePackageJson(),[addonName,versionSpecifier]=getVersionSpecifier(addon),{mainConfig,version:storybookVersion}=(0,import_core_common14.getStorybookInfo)(packageJson);if(!mainConfig){logger18.error("Unable to find storybook main.js config");return}let main=await(0,import_csf_tools10.readConfig)(mainConfig),addons=main.getFieldValue(["addons"]);addons&&!Array.isArray(addons)&&logger18.error("Expected addons array in main.js config"),logger18.log(`Verifying ${addonName}`);let latestVersion=packageManager.latestVersion(addonName);latestVersion||logger18.error(`Unknown addon ${addonName}`);let isStorybookAddon=addonName.startsWith("@storybook/"),addonWithVersion=`${addonName}@${versionSpecifier||(isStorybookAddon?storybookVersion:latestVersion)}`;logger18.log(`Installing ${addonWithVersion}`),packageManager.addDependencies({installAsDevDependencies:!0},[addonWithVersion]),logger18.log(`Adding '${addon}' to main.js addons field.`);let updatedAddons=[...addons||[],addonName];main.setFieldValue(["addons"],updatedAddons),await(0,import_csf_tools10.writeConfig)(main),options.skipPostinstall||await postinstallAddon(addon,isStorybookAddon)}var import_codemod=require("@storybook/codemod");async function migrate(migration,{glob,dryRun,list,rename,logger:logger26,parser}){if(list)(0,import_codemod.listCodemods)().forEach(key=>logger26.log(key));else if(migration)await(0,import_codemod.runCodemod)(migration,{glob,dryRun,logger:logger26,rename,parser});else throw new Error("Migrate: please specify a migration name or --list")}var import_path15=__toESM(require("path")),import_fs_extra10=require("fs-extra"),import_puppeteer_core=__toESM(require("puppeteer-core")),import_express=__toESM(require("express")),import_get_port=__toESM(require("get-port")),import_node_logger2=require("@storybook/node-logger"),read=async url=>{let browser=await usePuppeteerBrowser(),page=await browser.newPage();await page.goto(url),await page.waitForFunction(`
...
`)}
            `.trim(),{borderStyle:"round",padding:1,borderColor:"#F1618C"})),process.exit(1)),choices.length===1?[selectedTemplate]=choices:(logger21.info((0,import_boxen2.default)(import_ts_dedent24.dedent`
            ...
            `.trim(),{borderStyle:"round",padding:1,borderColor:"#F1618C"})),selectedTemplate=await promptSelectedTemplate(choices)),!!!(selectedTemplate??null)){logger21.error("Somehow we got no templates. Please rerun this command!");return}if(selectedConfig=allTemplates[selectedTemplate],!selectedConfig)throw new Error("\u{1F6A8} Sandbox: please specify a valid template type")}let selectedDirectory=outputDirectory,outputDirectoryName=outputDirectory||selectedTemplate;if(selectedDirectory&&(0,import_fs_extra11.existsSync)(`${selectedDirectory}`)&&logger21.info(`\u26A0\uFE0F  ${selectedDirectory} already exists! Overwriting...`),!selectedDirectory){let{directory}=await(0,import_prompts5.default)({type:"text",message:"Enter the output directory",name:"directory",initial:outputDirectoryName,validate:async directoryName=>(0,import_fs_extra11.existsSync)(directoryName)?`${directoryName} already exists. Please choose another name.`:!0},{onCancel:()=>{logger21.log("Command cancelled by the user. Exiting..."),process.exit(1)}});selectedDirectory=directory}try{let templateDestination=import_path16.default.isAbsolute(selectedDirectory)?selectedDirectory:import_path16.default.join(process.cwd(),selectedDirectory);logger21.info(`\u{1F3C3} Adding ${selectedConfig.name} into ${templateDestination}`),logger21.log("\u{1F4E6} Downloading sandbox template...");try{let gitPath=`github:storybookjs/sandboxes/${selectedTemplate}/${init?"after-storybook":"before-storybook"}#${branch}`;if(await(0,import_giget.downloadTemplate)(gitPath,{force:!0,dir:templateDestination}),(await(0,import_fs_extra11.readdir)(templateDestination)).length===0)throw new Error(import_ts_dedent24.dedent`Template downloaded from ${import_chalk25.default.blue(gitPath)} is empty.
          Are you use it exists? Or did you want to set ${import_chalk25.default.yellow(selectedTemplate)} to inDevelopment first?`)}catch(err){throw logger21.error(`\u{1F6A8} Failed to download sandbox template: ${err.message}`),err}let initMessage=init?import_chalk25.default.yellow(`yarn install
yarn storybook`):`Recreate your setup, then ${import_chalk25.default.yellow("run npx storybook init")}`;logger21.info((0,import_boxen2.default)(import_ts_dedent24.dedent`
